...
exports.sourceNodes = async ({ boundActionCreators, getNode, hasNodeChanged, store, cache }, { baseUrl, apiBase, basicAuth }) => {
  const { createNode } = boundActionCreators;

  // Default apiBase to `jsonapi`
  apiBase = apiBase || `jsonapi`;

  // Provide auth defaults.
  axios.defaults.auth = {
    username: basicAuth.htaccess_user,
    password: basicAuth.htaccess_pass
  };
    ...
    if (node.internal.type === `files` || node.internal.type === `file__file`) {
      try {
        ...
        fileNode = await createRemoteFileNode({
          ...
          createNode,
          auth: {
            htaccess_user: basicAuth.htaccess_user,
            htaccess_pass: basicAuth.htaccess_pass
          }
        });
      } catch (e) {
        // Ignore
